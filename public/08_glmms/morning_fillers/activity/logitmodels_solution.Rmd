...
tclean_bin <- tclean_af %>%
  mutate(onset_bin = floor((onset_slog + .125) / .25) * .25) %>%
  group_by(onset_bin) %>%
  summarise(p = mean(anyfill), N = n())
```
...
```{r T08}
g <- ggplot(tclean_bin, aes(onset_bin, p)) +
  geom_point(aes(size = N), alpha = .2) 
...
```{r T09, cache = TRUE}
mod <- glmer(anyfill ~ onset_slog + 
               (onset_slog | subj_id) + 
               (onset_slog | pic_id), 
             binomial(link = "logit"), data = tclean_af)
```
...
```{r T10}
newdat <- tibble(onset_slog = seq(-1, 3, .01))
```
...
```{r T11}
mod_pred <- newdat %>%
  mutate(p = predict(mod, newdat, re.form = ~ 0, type = "response"))
```
...
```{r T12}
g + geom_line(data = mod_pred %>% rename(onset_bin = onset_slog))
```
...
```{r T13}
pause_len <- exp(-fixef(mod)[1] / fixef(mod)[2])
```
